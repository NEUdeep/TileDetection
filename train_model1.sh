...
bash ./tools/dist_train.sh cascade_s50_rfp_mstrain_augv2-B.py $GPUS --no-validate --seed=10 --deterministic

sleep 10s

echo 'Begin swa training for first model.'
bash ./tools/dist_train.sh swa_cascade_s50_rfp_mstrain_augv2.py $GPUS --no-validate --seed=9 --deterministic